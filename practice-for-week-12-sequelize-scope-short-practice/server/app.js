...
app.get('/instruments/keyboard', async (req, res, next) => {
    const keyboards = await Instrument.scope('defaultScope', { method: ["isInstrumentType", "keyboard"] }).findAll()
    res.json(keyboards);
});


app.get('/instruments/string', async (req, res, next) => {
    const strings = await Instrument.scope('defaultScope', { method: ["isInstrumentType", "string"] }).findAll()
    res.json(strings);
});

app.get('/instruments/woodwind', async (req, res, next) => {
    const woodWinds = await Instrument.scope('defaultScope', { method: ["isInstrumentType", "woodwind"] }).findAll()
    res.json(woodWinds);
});
...
app.get('/stores/:storeId/instruments', async (req, res, next) => {
    ...
    const filterStoreInstruments = await Instrument.scope('defaultScope',{method:['instrumentByStore', storeId]}).findAll({
        order:[['name']]
    })
